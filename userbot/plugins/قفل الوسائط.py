...
async def _(event):  
    ...
    if admincheck:
        ...
        output += f"**صلاحيات الادمن **{_format.mentionuser(user.first_name ,user.id)} **في {event.chat.title} هذه المجموعة ⚜ **\n"
        ...
        try:
            ...
            ucpin = "❌" if chat_per.pin_messages else "✅"
            uchangeinfo = "❌" if chat_per.change_info else "✅"
        output += f"{_format.mentionuser(user.first_name ,user.id)} **صلاحياته في{event.chat.title} المجموعه هي **\n"
